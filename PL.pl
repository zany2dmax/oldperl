...
#
$DBMH = DBI->connect('DBI:mysql:servers;hermes.corp.interland.net','N6ix$Vkn9') or die "connecting : $DBI::errstr\n";
my $OASIS_SHOWS_DEAD = 0;
foreach my $SERVER (%OFFLINESERVERS) {
   my $AGENTNAME = $DBMH->quote($SERVER);
   my $OASISPULL = $DBMH->prepare("select PhysicalName,Status from Oasis__Active__ where Physicalname = $AGENTNAME");
   $OASISPULL->execute();
   while (($SERVER, my $STATUS)=$OASISPULL->fetchrow_array) {
      if ($STATUS ne "Active") {
